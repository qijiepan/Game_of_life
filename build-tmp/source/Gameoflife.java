  ...
  public void init(){
    ...
    for (int i =1; i< columns-1;i++){
      for(int j=1;j<rows-1;j++){
        ...
        if(test<=0.05f) board[i][j]=4; // the probility of disease
        else if(test<0.4f) board[i][j]=0; // the probility of dead prey
        else if(test<0.6f) board[i][j]=-1; // the probility of dead predator
        else if(test<0.75f) board[i][j]=1; // the probility of alive predator
